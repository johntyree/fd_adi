    ...
    # def solve(self, n, dt):
        ...
        # for t in range(1, n+1):
            ...
            # for i in range(v.shape[0]-2, 0, -1):
                # v[i] = (dt*(gamma2[i] + mu[i])*v[1 + i] + dt*(gamma2[i] - mu[i])*vm[-1 + i] +
                    # (1 - dt*r - dt*gamma2[i] + dt*mu[i])*vm[i])/(1 + dt*(gamma2[i] + mu[i]))
            # duds = mu[-1] * self.grid.mesh[0][-1]
            # d2uds = gamma2[-1]*(self.grid.mesh[0][-1]*2*dx + 2*vm[-2] - 2*vm[-1])
            # v[-1] = vm[-1] + dt*(duds + d2uds + -r*vm[-1])
            # v1 = v
            ...
            # v[-1] = vm[-1] + dt*(duds + d2uds + -r*vm[-1])
            # for i in range(1, v.shape[0]-1):
                # v[i] = (vm[i] - dt*(r*vm[i] + mu[i]*(v[-1 + i] + vm[i] - vm[1 + i])) +
                           # dt*gamma2[i]*(v[-1 + i] - vm[i] + vm[1 + i]))/(1 + dt*gamma2[i] - dt*mu[i])
            # v2 = v
            # v = (v1+v2)/2
            # self.grid.domain.append(v)
